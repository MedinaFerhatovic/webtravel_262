...
export class AdminUserComponent implements OnInit {
  ...
  changeUserStatus(user: User): void {
    const newActiveStatus = !user.status;

    this.userService.changeUserStatus(user._id, newActiveStatus).subscribe(
        (updatedUser: User) => {
        ...
        (error) => {
            console.error('Promjena aktivnog statusa korisnika nije uspjela: ', error);
        }
    );
}
